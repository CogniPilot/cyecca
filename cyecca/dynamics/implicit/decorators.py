...
def implicit(cls):
    ...
    Example:
        ...
        >>> x.theta  # ImplicitVar variable
        >>> x.theta.dot()  # Derivative symbol (marks theta as state)
        >>>
    ...
    def create_symbolic(cls_obj, sym_type=ca.SX):
        ...
        for field_name, info in cls_obj._field_info.items():
            ...
            if var_type == 'time':
                # Time is special - just store None for symbolic, gets populated in simulate()
                ...
                setattr(instance, field_name, v)
                instance._field_names.append(field_name)
            elif var_type in ('state', 'alg'):
                raise ValueError(
                    f"Field '{field_name}' uses '{var_type}()' which is not allowed in implicit models. "
                    f"Use var() instead - states are automatically inferred from .dot() usage."
                )
            # Skip unknown types
    ...
    def getitem(self, idx):
        ...
        Example:
            >>> t, data = model.simulate(0, 10, 0.01)
            >>> final = data[-1]  # Get final state
            >>> initial = data[0]  # Get initial state
            >>> 
            >>> # Continue simulation from final state
            >>> model.v0 = data[-1]
        """
        ...
        for name in self._field_names:
            ...
            # Skip time fields - time is built-in now
            if info["type"] == "time":
                continue
                
            # Skip if field not in trajectory (e.g., time was skipped)
