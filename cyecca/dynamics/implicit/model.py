...
1. STRONG TYPE SAFETY
   ...
   - Field descriptors (var(), param()) enforce variable categorization
   - Variable types inferred from usage (.dot() → state, no .dot() → algebraic)
   - DO NOT remove type hints or @beartype decorators in future edits

2. AUTOCOMPLETE-FRIENDLY API
   - All variables accessible via model.v namespace
   - Numeric state accessible via model.v0 namespace
   - ImplicitVar wrapper provides .dot() for derivatives with autocomplete
   - __dir__() methods ensure IDE autocomplete shows only relevant fields
   - DO NOT replace typed namespaces with dictionaries or untyped access

3. MODELICA-STYLE UNIFIED NAMESPACE
   - All variables (states, algebraic, params) in one class
   ...
   - Variable classification inferred from .dot() usage (like Modelica)
   - DO NOT split into separate state/algebraic/param classes

================================================================================
                     MODELICA DAE REPRESENTATION  
================================================================================

This module implements IMPLICIT DAE models following the Modelica DAE
representation (Appendix B of Modelica Specification).
...
where v := [p; t; ẋ; x; y; z; m; pre(z); pre(m)] and:
    ...
    - x(t): Differentiated variables (states) - have .dot() called
...
--------------------------------------------------------------------------------
                     IMPLICIT DAE SUBSET (This Module)
--------------------------------------------------------------------------------

This module implements the continuous-time implicit DAE subset:

    0 = f_x(ẋ, x, y, p, t)           Differential-algebraic residual

where:
    - x ∈ ℝⁿ: State vector (variables with .dot() called)
    - ẋ ∈ ℝⁿ: State derivative vector
    - y ∈ ℝᵏ: Algebraic variables (variables without .dot())
    - p ∈ ℝˡ: Parameter vector (variables marked with param())
    - t ∈ ℝ: Time (built-in, accessed as model.t)

Variable classification is AUTOMATIC based on usage:
    - var() with .dot() called in equations → STATE (x)
    - var() without .dot() called → ALGEBRAIC (y)
    - param() → PARAMETER (p)

Function naming follows Modelica convention:
    - model.f_x: 0 = f_x(ẋ, x, y, p, t) - DAE residual function

For index-1 DAEs, the system can be solved by:
    1. Solving algebraic equations for y given x
    2. Computing ẋ from the differential equations
...
================================================================================
    ...
    model.v.theta        # variable (symbolic)
    model.v.theta.dot()  # derivative (symbolic) - MARKS theta AS STATE!
    model.v.l            # parameter (symbolic)
    model.t              # time (built-in)
    
    # Define implicit equations: 0 = f_x(ẋ, x, y, p, t)
    model.eq(model.v.theta.dot() - model.v.omega)                    # dθ/dt = ω
    model.eq(model.v.omega.dot() + model.v.g/model.v.l * ca.sin(model.v.theta))
    
    model.build()  # Auto-infers: theta, omega are states (have .dot())
    
    # Numeric namespace (for simulation):
    model.v0.theta = 0.5  # Set initial state
    
    # Simulate
    t, data = model.simulate(0.0, 10.0, 0.01)
    # data.theta is an ndarray
...
class Model(Generic[TModel]):
    ...
    t: ca.SX  # Time (built-in, like Modelica) - type hint for autocomplete
    ...
    def build(self):
        ...
        for eq in self.equations:
            ...
            depends_on_xdot = any(ca.is_equal(var, xdot_vec[i]) for i in range(xdot_vec.shape[0]) for var in eq_vars)
        ...
        if len(ode_residuals) != n_states:
            raise ValueError(
                f"ODE equation count mismatch: {len(ode_residuals)} ODE equations " f"for {n_states} states."
            )
    ...
    def _extract_all_values(self, v):
        """Extract all field values as a flat dict."""
        values = {}
