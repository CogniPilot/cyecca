...
from cyecca.sim import msgs, uros
from cyecca.sim.uros import Publisher


# Only apply beartype if pyulog is available to avoid forward reference issues
if HAS_PYULOG:
    @beartype
    class LogEvent:
        def __init__(self, timestamp: np.uint64, index: int, topic: 'pyulog.ULog.Data'):
    ...
    # Stub version when pyulog is not available
    class LogEvent:
        def __init__(self, timestamp: Any, index: int, topic: Any):
            raise ImportError(
                'pyulog is required for LogEvent. '
                'Install with: pip install 'cyecca[estimator]' or pip install pyulog'
            )
...
class ULogReplay:
    def __init__(self, core: uros.Core, ulog_file: str):
        if not HAS_PYULOG:
            raise ImportError(
                'pyulog is required for ULogReplay. '
                'Install with: pip install 'cyecca[estimator]' or pip install pyulog'
            )
